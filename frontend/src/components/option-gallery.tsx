...
export function OptionGallery({ options, onToggleReady, onArchive }: OptionGalleryProps) {
  if (options.length === 0) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {options.map((opt) => (
        <OptionCard key={opt.id} option={opt} onToggleReady={onToggleReady} onArchive={onArchive} />
      ))}
    </div>
  );
}
