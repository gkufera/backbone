...
function makeExisting(overrides: Partial<ExistingElement> & { name: string }): ExistingElement {
  return {
    ...
    pageNumbers: overrides.pageNumbers ?? [1],
  };
}
...
describe('Element Matcher', () => {
  ...
  it('handles mixed: exact + fuzzy + new + missing', () => {
    ...
    const fuzzy = result.matches.filter((m) => m.status === 'FUZZY');
    const newElems = result.matches.filter((m) => m.status === 'NEW');

    expect(exact).toHaveLength(1);
    expect(exact[0].detectedName).toBe('JOHN SMITH');

    // JOHN SMITHE should fuzzy match MARY JONES? No - let's check similarity.
    // Actually JOHN SMITHE vs MARY JONES is very different.
    // JOHN SMITHE has no good fuzzy match since JOHN SMITH is consumed.
    // So it should be NEW along with XAVIER
